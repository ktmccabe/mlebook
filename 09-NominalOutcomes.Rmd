...
The model we will use to estimate this is the multinomial logit.
  ...
  - $\beta_J = 0$ for identification ($J$ represents the baseline category); 
  - $\sum_{k=1}^{J} \pi_k  = 1$, The probabilities of being in each category, together, must sum to 1.
...
\begin{align*}
\pi_j = Pr(Y_i = C_j | x_i) &= \frac{\exp(\mathbf x_i^T\beta_j)}{1 + \sum_{k=1}^{J-1} \exp(\mathbf x_i^T\beta_k)}
\end{align*}


### Multinomial Likelihood

Here, similar to the ordinal log likelihood, we need to sum over all observations and all outcome categories:
...
Predicted probabilities of the outcome being in a particular outcome category $C_j$: 
  ...
  - $Pr(Y_i = C_j | X) = \frac{\exp(X\hat \beta_j)}{1 + \sum_{k=1}^{J-1} \exp(X \hat \beta_k)}$
      + The numerator is very similar to the numerator for when we have a binary logistic regression
      + The denominator sums up $\exp(X \hat \beta_k)$ where $\hat \beta_k$ represents the set of coefficients for each of the other outcome categories except for the baseline (i.e., $J- 1$ means 1 less than the total number of outcome categories).
...
Just like in the other models, we can rely on outside packages, too. When we use `margins` or `prediction`,. For some models, these packages are not going to have full capabilities-- they might not be able to calculate standard errors, for example. These are "living packages" so you can always check the documentation and update the packages to see if new capabilities have been added.
...
We can use `margins` to calculate the difference in predicted probabilities, for example, between `female=1` and `female=0`. We should specify the category for which we want this comparison.
...
If we wanted uncertainty estimates around our quantities of interest, we could use the bootstrap!
