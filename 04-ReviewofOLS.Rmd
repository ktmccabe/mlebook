...
Key variables include 
  ...
  - The data include several other variables related to the demographics of the respondents and measures representing potential alternative explanations, such as `JobComp_cont` (agreement with "the inflow of young
male refugees makes it more difficult for young native men to find apprenticeships and jobs") and `LifeSatis_cont` (0-10 scale, ranging from extremely dissatisfied to extremely satisfied).
...
<details> <summary> Try to code these on your own, then click for the solution</summary>
```{r}
...
<details> <summary>Try on your own, then click for the solution</summary>
```{r}
summary(lm1)
summary(lm2)
```
...
**Additional Models**
We can also run regressions with even more covariates, as the authors do in models 3-6 in the paper. 

<details> <summary>Click to reveal regression code below.</summary>
...
Even with all these covariates accounted for, the authors still engage in a discussion about possible violations of the OLS assumptions that could bias their results, as well as potential alternative modelling strategies.
  ...
  - Validity (discussed in Gelman and Hill). Does the outcome accurately measure the concept? They consider alternative outcomes and visualize the coefficient results in Figure 4.
    ...
    + Justified : Hostility against refugees is sometimes justified, even when
it ends up in violence.
    + Prevent : Xenophobic acts of violence are defensible if they result in fewer
refugees settling in town.
