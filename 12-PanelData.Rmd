...
```
With a small p-value, we reject the null, suggesting fixed effects is preferred for this model. Note: while the test provides one way to adjudicate between fixed and random effects, it cannot tell you if the model you have specified is "correct."

### Using lme4 for random effects

More flexible package for random effects: `lme4`. Uses REML by default- variation on ML. Ben Bolker provides a great overview of this [here](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html)
...
In your own research you might ask the following questions:
  ...
  - Are my observations independent?
      + Then, you might be fine with a standard model
  - Is there some grouping structure to my data?
...
Then, you might consider the following choices:
...
What we have not yet discussed are the following issues. See links for further study.
  ...
  - Incorporating dynamics into longitudinal data: $Y_it = \alpha_i + \rho Y_{i t-1} + \beta x_{it} + \epsilon_{it}$
