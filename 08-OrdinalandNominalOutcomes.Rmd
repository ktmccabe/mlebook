...
For example, let's fit the authors' model as a linear model using MacKinnon and White HC1 robust standard errors. Those specify the following inside portion for an estimate of $\mathbf{V}( \epsilon)$.This is what Stata uses as a default:
...
Let's estimate the probability of being in the lowest category for empathy values from 0 to 1 by .2 intervals. Let's set all covariates at their means.

  - We could do this in `predict` or manually. We will do it manually for now.

```{r}
## Set covariates to particular values
X <- model.matrix(fit.empw2)
X <- X[, -1] #remove intercept

## Crude way to take means 
## why might this be bad to do in practice?
X.means <- colMeans(X)
X.means["empconc"] <- 0

# Find Xb and zeta
# this piece [1:length(X.means)] makes sure we omit the zetas
eta <- X.means %*% coef(fit.empw2)[1:length(X.means)]
zeta <- fit.empw2$zeta

## Find Pr(lowest category)
emp0 <- plogis(zeta[1] - eta)
emp0
```
...
findprall <- function(val){
  ...
  pr7 <- 1 - plogis(zeta[6] - eta)
  ## Find Pr(4th lowest category)
  pr6 <- plogis(zeta[6] - eta) - plogis(zeta[5] - eta)
  ## Find Pr(4th lowest category)
  pr5 <- plogis(zeta[5] - eta) - plogis(zeta[4] - eta)
  ## Find Pr(4th lowest category)
  pr4 <- plogis(zeta[4] - eta) - plogis(zeta[3] - eta)
  ## Find Pr(3rd lowest category)
  pr3 <- plogis(zeta[3] - eta) - plogis(zeta[2] - eta)
  ## Find Pr(2nd lowest category)
  pr2 <- plogis(zeta[2] - eta) - plogis(zeta[1] - eta)
  ## Find Pr(lowest category)
  pr1 <- plogis(zeta[1] - eta)
...
A last step would be to calculate uncertainty. Just like before, we could use simulation or the bootstrap method.
