...
Sometimes in order to answer our research questions, we need to combine data from multiple sources. Ultimately, however, we really do need to ***combine*** them. If we have a large amount of data, this may be quite daunting. Fortunately, R has several commands that allow us to merge or append datasets.

Here are a few resources on merging and appending data:

  - Using the `merge` command in R. [Tutorial](https://sejdemyr.github.io/r-tutorials/statistics/tutorial5.html)
  ...
  - Appending data in R (e.g., Maybe you have one dataset from 2010 and another from 2012, and you want to stack them on top of each other). [Explanation](https://www.princeton.edu/~otorres/Merge101R.pdf)
